...
static mk_task_handle_t spawn_nextos(const char* name, uint8_t prio, size_t stack,
    ...
    mk_watchdog_register(name, 4000, nullptr);
    ESP_LOGI(TAG, "[PASS] %s task %s prio=%u core=%d", MK_CONFIG_OS_NAME, name, (unsigned)prio, core);
    return h;
}
...
esp_err_t SystemController::initialize(){
    ...
    // 4. Storage & Settings
    // NVS is non-fatal: if the partition is missing/corrupt we still bring up
    // the display with factory defaults. A hard failure here previously left the
    // panel in its post-reset scan state (the blue-line "noise" symptom).
    ...
    // The panel is write-only, so successful initialization proves only that
    // the ESP-side SPI transactions completed; there is no readable device ID.
    ...
    // 8. Time Service (non-fatal - display works without SNTP)
...
static void gui_thread(void* arg){
    ...
    while(true){
        ...
        uint32_t delay = runtime.handle_timer();
        // LVGL can request another draw on the very next 1 ms tick while
        // animations or frequent status invalidations are active. At GUI
        // priority 15 that can continuously preempt IDLE1 and trip the task
        // watchdog even though each handler pass completes. Reserve a small
        // blocking window for the CPU1 idle task; 5 ms remains well above the
        // refresh capability of the 8 MHz SPI panel.
        if(delay < 5) delay = 5;
...
static void system_thread(void* arg){
    ...
    while(true){
        ...
        if(now - last_ui > 500){
            ...
            char time_buf[16], date_buf[16], time_full[16];
