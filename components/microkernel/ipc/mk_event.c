...
#if MK_NATIVE_KERNEL
struct mk_event_group { uint32_t bits; portMUX_TYPE lock; };
mk_event_group_t* mk_event_group_create(const char* name){
    (void)name;
    mk_event_group_t* g = heap_caps_malloc(sizeof(*g), MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT);
    if(!g) g=malloc(sizeof(*g));
    if(!g) return NULL;
    g->bits=0; g->lock=0;
    return g;
}
mk_status_t mk_event_group_delete(mk_event_group_t* group){ if(!group) return MK_ERR_INVALID; heap_caps_free(group); return MK_OK; }
mk_event_bits_t mk_event_set(mk_event_group_t* group, mk_event_bits_t bits){ if(!group) return 0; taskENTER_CRITICAL(&group->lock); group->bits|=bits; uint32_t r=group->bits; taskEXIT_CRITICAL(&group->lock); return r; }
mk_event_bits_t mk_event_clear(mk_event_group_t* group, mk_event_bits_t bits){ if(!group) return 0; taskENTER_CRITICAL(&group->lock); group->bits&=~bits; uint32_t r=group->bits; taskEXIT_CRITICAL(&group->lock); return r; }
mk_event_bits_t mk_event_wait(mk_event_group_t* group, mk_event_bits_t bits_to_wait, bool clear_on_exit, bool wait_all, uint32_t timeout_ms){
    ...
    while(1){
        ...
        taskEXIT_CRITICAL(&group->lock);
        if(esp_timer_get_time() >= until) return group->bits & bits_to_wait;
        mk_port_delay_ms(1);
    }
}
mk_event_bits_t mk_event_get(mk_event_group_t* group){ if(!group) return 0; return group->bits; }
#else
