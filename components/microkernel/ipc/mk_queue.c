...
#if MK_NATIVE_KERNEL
// Native circular buffer queue — no FreeRTOS
struct mk_queue {
    uint8_t* buf;
    size_t cap;
    size_t item_size;
...
mk_queue_t* mk_queue_create(size_t length, size_t item_size, const char* name){
    (void)name;
    mk_queue_t* q = heap_caps_malloc(sizeof(*q), MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT);
    if(!q) q=malloc(sizeof(*q));
    if(!q) return NULL;
    q->buf = heap_caps_malloc(length*item_size, MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT);
    if(!q->buf){ heap_caps_free(q); return NULL; }
    q->cap=length; q->item_size=item_size; q->head=0; q->tail=0; q->count=0; q->lock=0;
    return q;
}
mk_status_t mk_queue_delete(mk_queue_t* queue){ if(!queue) return MK_ERR_INVALID; heap_caps_free(queue->buf); heap_caps_free(queue); return MK_OK; }
mk_status_t mk_queue_send(mk_queue_t* queue, const void* item, uint32_t timeout_ms){
    ...
    while(1){
        ...
        if(queue->count < queue->cap){
            ...
            queue->tail=(queue->tail+1)%queue->cap; queue->count++;
...
mk_status_t mk_queue_receive(mk_queue_t* queue, void* item, uint32_t timeout_ms){
    ...
    while(1){
        ...
        if(queue->count>0){
            ...
            queue->head=(queue->head+1)%queue->cap; queue->count--;
            taskEXIT_CRITICAL(&queue->lock);
            return MK_OK;
        }
        taskEXIT_CRITICAL(&queue->lock);
...
mk_status_t mk_queue_send_isr(mk_queue_t* queue, const void* item){
    // ISR safe — try without block
    if(!queue||!item) return MK_ERR_INVALID;
    bool ok=false;
    taskENTER_CRITICAL(&queue->lock);
    if(queue->count < queue->cap){
        memcpy(queue->buf + queue->tail*queue->item_size, item, queue->item_size);
        queue->tail=(queue->tail+1)%queue->cap; queue->count++; ok=true;
    }
    taskEXIT_CRITICAL(&queue->lock);
...
mk_status_t mk_queue_peek(mk_queue_t* queue, void* item, uint32_t timeout_ms){
    ...
    if(!queue||!item) return MK_ERR_INVALID;
    taskENTER_CRITICAL(&queue->lock);
    if(queue->count==0){ taskEXIT_CRITICAL(&queue->lock); return MK_ERR_TIMEOUT; }
    memcpy(item, queue->buf + queue->head*queue->item_size, queue->item_size);
    taskEXIT_CRITICAL(&queue->lock);
    return MK_OK;
}
size_t mk_queue_get_count(mk_queue_t* queue){ if(!queue) return 0; return queue->count; }
bool mk_queue_is_full(mk_queue_t* queue){ if(!queue) return true; return queue->count >= queue->cap; }
#else
