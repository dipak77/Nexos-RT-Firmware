...
mk_status_t mk_svc_dispatch(mk_svc_num_t svc_num, mk_svc_args_t* args, uintptr_t* out_result) {
    ...
    // 1. Enforce Capability Validation (Claim 3)
    if (enclave != NULL && required_cap != MK_CAP_NONE) {
        if ((enclave->syscall_mask & required_cap) != required_cap) {
            ...
            mk_enclave_trap(enclave, (uint16_t)svc_num, "capability_violation");
        ...
        if (svc_num == MK_SVC_GFX_FLUSH || svc_num == MK_SVC_NET_SEND) {
            ...
            if (buf && len > 0) {
                if (!mk_svc_validate_ptr(enclave, buf, len)) {
                    ...
                    mk_enclave_trap(enclave, (uint16_t)svc_num, "oob_memory_access");
