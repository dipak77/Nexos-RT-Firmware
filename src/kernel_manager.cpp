...
mk_task_handle_t KernelManager::spawn(const char* name, uint8_t prio, size_t stack,
    ...
    mk_task_handle_t h = mk_task_create_ext(&cfg, entry, nullptr);
    if (!h) {
        Serial.printf("[FAIL] spawn %s prio=%u core=%d\n", name, (unsigned)prio, core);
        return nullptr;
    }
    mk_watchdog_register_task(h, 4000, nullptr);
    Serial.printf("[PASS] %s task %s  prio=%u  core=%d  stack=%u\n",
